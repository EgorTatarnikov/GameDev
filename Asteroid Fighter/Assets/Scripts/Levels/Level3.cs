...
public class Level3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isPartFinished[2] && !isPartFinished[3])        //supernovas
        {
            Part3();
            if (spawnCount >= 27)
            {
        ...
        if (isPartFinished[4] && !isPartFinished[5])        //supernovas
        {
            Part5();
            if (spawnCount >= 27)
            {
        ...
        if (isPartFinished[7] && !isPartFinished[8])
        {
            if (!isPartStarted[8])
            {
                spawnTimer.Stop();
                spawnTimer.Duration = 5;
                isPartStarted[8] = true;
                spawnTimer.Run();
            }

            if (GMScript.Boss3Killed == true)
            {
        ...
        if (isPartFinished[8])
        {
            if (spawnTimer.Finished)
            {
                ...
                asteroidSpawner.InfiniteLevel();
            }
        }

        cometSpawnTime += Time.deltaTime;
        if (cometSpawnTime >= 33.5f && cometSpawned < 4)
        {
    ...
    void Part7()
    {
        ...
        {
            spawnCount++;
            Object.Instantiate(Resources.Load("Boss3"), transform.position, Quaternion.identity);
        }
    }
